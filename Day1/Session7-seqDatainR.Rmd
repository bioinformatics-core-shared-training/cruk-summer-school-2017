...
The region filer can be used in conjuction with the `what` argument to `ScanBamParam` function to provide a detailed picture of the reads for your gene. First we need to define a region:-
...
```{r echo=FALSE}
mygene.reads <- readGAlignments(file="/home/participant/Course_Materials/data/test/paired.bam",
...
- Extract the depth values for each position, in order to plot the read coverage over this genomic region
- Navigate to `chr17:7577851-7598063` in IGV
    + does the plot roughly agree?
- How many positions in this region have a depth of over 10?
