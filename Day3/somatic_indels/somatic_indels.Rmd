...
* Short exercise: exploring indels in TGF-beta receptor 2
...
* 1000 Genomes Project <span style="font-size: 75%">[[Nature 2010](http://www.ncbi.nlm.nih.gov/pubmed/20981092)]</span> --- loss of function indel variants in each individual
...
* Recent Sanger Institute paper on 560 Breast Cancers <span style="font-size: 75%">[[Nik-Zainal et al., Nature 2016](https://www.ncbi.nlm.nih.gov/pubmed/27135926)]</span> searched for novel indel drivers in non-coding regions with significant recurrence (functional regulatory elements)
...
* <span>Application of 1000 Genomes Project data to cancer genomics <span style="font-size: 75%">[[Science 2013](https://www.ncbi.nlm.nih.gov/pubmed/27135926)]</span> showed that genes linked with cancer showed stronger selection against indels</span>
...
* <span>Homopolymer A/T indels considered to be major source of low quality indel calls<span style="font-size: 75%">[[Fang et al., Genome Med. 2014](https://www.ncbi.nlm.nih.gov/pubmed/25426171)]</span></span>
...
highchart(width = 900, height = 450) %>%
  hc_chart(type = "column", animation = TRUE, zoomType = "x", backgroundColor = NULL) %>%
  hc_xAxis(categories = counts$size, title = list(text = "Size (bp)")) %>%
  hc_yAxis(title = list(text = "")) %>%
  hc_add_series(name = "insertions", data = counts$insertions, showInLegend = TRUE) %>%
  hc_add_series(name = "deletions", data = counts$deletions, showInLegend = TRUE) %>%
  hc_colors(c("#79b4ec", "#ff3385")) %>%
...
repeat_expansion_contraction <- repeat_expansion_contraction %>%
  spread(type, n) %>%
  mutate(contraction = ifelse(is.na(contraction), 0, contraction)) %>%
  mutate(expansion = ifelse(is.na(expansion), 0, expansion))

highchart(height = 450) %>%
  hc_chart(type = "column", animation = TRUE, zoomType = "x", backgroundColor = NULL) %>%
  hc_xAxis(categories = repeat_expansion_contraction$seq, title = list(text = "Repeat")) %>%
  hc_yAxis(title = list(text = "")) %>%
  hc_add_series(name = "expansion", data = repeat_expansion_contraction$expansion, showInLegend = TRUE) %>%
  hc_add_series(name = "contraction", data = repeat_expansion_contraction$contraction, showInLegend = TRUE) %>%
  hc_colors(c("#79b4ec", "#ff3385")) %>%
  hc_tooltip(formatter = JS("function() { return (this.x + ': ' + this.y) }"))
```
